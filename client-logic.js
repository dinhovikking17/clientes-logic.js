(function() {
    ...
    function applyTextReplacements(replacements) {
        ...
        for (const key in replacements) {
            ...
            if (item && item.original && item.new) {
    ...
    function hideExistingLogos() {
        const protectedLogoIds = ['panel-injected-logo', 'appLogoImageNewUI', 'appLogoContainerNewUI'];
        const selector = '[id*="logo" i], [class*="logo" i], img[src*="logo" i], img[alt*="logo" i]';
        try {
            document.querySelectorAll(selector).forEach(logoElement => {
                if (!protectedLogoIds.includes(logoElement.id)) {
                    logoElement.style.setProperty('opacity', '0', 'important');
                    logoElement.style.setProperty('pointer-events', 'none', 'important');
                }
            });
        } catch (e) {
    ...
    (async function() {
        ...
        try {
            const cachedData = localStorage.getItem(CACHE_CONFIG_KEY);
            if (cachedData) {
                cachedConfig = JSON.parse(cachedData);
                hideExistingLogos();
                applyCustomBackground(cachedConfig.uiCustomization);
                applyLogo(cachedConfig.logoConfig);
                applyTextReplacements(cachedConfig.textReplacements);
            }
        } catch (e) {
            console.warn("Painel: Não foi possível carregar ou aplicar a configuração do cache.", e);
        }
        ...
        try {
            ...
            firebase.database().ref('configs/' + MY_UNIQUE_USER_ID).on('value', (snapshot) => {
                ...
                try {
                    ...
                    console.warn("Painel: Não foi possível salvar a configuração no cache.");
                }
                processConfig(remoteConfig);
            });
        } catch (error) {
            console.error("Painel: Erro ao conectar ao Firebase. Usando dados do cache como fallback.", error);
            if (cachedConfig) {
                await processConfig(cachedConfig);
            }
        }
    })();
